...
def gradient_descent_visualization(X, y, k_init, a, it):    
    ...
    axis[0].legend(loc='best')
    axis[0].text(0.5, 1.5, s="$k$="+f"{k:.4}", va='bottom', ha='left')
    plt.show() 
...
def plot_function_in_3d(angle1, angle2):
    ...
    ax.set_zlabel('$\phi(x, y)$', labelpad=20)
...
def plot_3d_func_with_grad(x0=0, y0=0, pos_neg_grad=None):    
    ...
    if pos_neg_grad is not None:
        ...
        plt.text(-10, -6.5, "$\dfrac{\delta \phi(x_0, y_0)}{\delta x} = 4.5x_0 + 7.5 = " + "{0} $".format(grad[0]), ha='left', va="center")
        plt.text(-10, -9, "$\dfrac{\delta \phi(x_0, y_0)}{\delta y} = 5y_0 = " + "{0} $".format(grad[1]), ha='left', va="center")
        ...
        if pos_neg_grad in 'positive':
            col = 'black'     
            plt.text(x0 + grad[0], y0 + grad[1], "$(x_0 + \dfrac{\delta \phi(x_0, y_0)}{\delta x}, y_0 + \dfrac{\delta \phi(x_0, y_0)}{\delta y})$", ha='center', va="bottom", color=col)                
            plt.text(x0 + grad[0]/2, y0, "$\dfrac{\delta \phi(x_0, y_0)}{\delta x}$", ha='center', va='top')
            plt.text(x0, y0 + grad[1]/2, r"$\dfrac{\delta \phi(x_0, y_0)}{\delta y}$", va='center', ha="right")
        ...
        if pos_neg_grad in 'negative':
            ...
            #plt.text(x0 - grad[0]/2, y0, "$-\dfrac{\delta \phi(x_0, y_0)}{\delta x}$", ha='center', va='bottom')
            #plt.text(x0, y0 - grad[1]/2, r"$-\dfrac{\delta \phi(x_0, y_0)}{\delta y}$", va='center', ha="left")
            ...
            plt.text(x0 - grad[0], y0 - grad[1], "$(x_0 - \dfrac{\delta \phi(x_0, y_0)}{\delta x}, y_0 - \dfrac{\delta \phi(x_0, y_0)}{\delta y})$", ha='center', va="top", color='black')
...
def plot_gradient_descent_in_3d(X, y, iters=5, alpha=0.15):    
    ...
    plt.scatter(ks[-1], bs[-1], s=50, color='r', label="Конечные значения")
