...
def plot_dividing_line_iris(X, kind_iris):
    theta = np.array([ 0.25,  0.94, -1.7])
    create_base_plot()
    teta_0, teta_1, teta_2 = theta
    plt.scatter(X[:,0][kind_iris=='iris setosa'], X[:,1][kind_iris=='iris setosa'], label='iris setosa', c='g')
    plt.scatter(X[:,0][kind_iris=='iris virginica'], X[:,1][kind_iris=='iris virginica'], label='iris virginica', c='b')
...
def plot_sigmoid():
    ...
    y = 1/(1+np.e**-x)
    plt.plot(x,y, label="$\sigma(X_i\Theta)$")
    plt.xlabel("$X_i\Theta$", fontsize=17)
...
def plot_div_mse_ce():
    ...
    plt.grid()
    plt.xlabel("$\sigma(X_i\Theta)$", fontsize=17)
    plt.legend()
    plt.show()
